...
def run_feature_extraction(
    ...
    video_dir = os.path.join(data_root, "videos", "val")
    ann_path  = os.path.join(data_root, "annotations", "thumos_14_anno.json")
...
def flush_features(
    ...
    if complete_only and len(keys) > 1:
        keys = keys[:-1]
    for vname in keys:
        feats = torch.stack(video_features.pop(vname), dim=0)
